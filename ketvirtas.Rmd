...
plot_nedarbas_region <- ggplot(data3, aes(Metai, NedarboLygis)) +   
  geom_line(aes(colour = Regionas), size = 1) + 
  labs(title = "Nedarbo lygio dinamika pagal apskritis 2011-2016 m.",
       x = "Laikas", y = "Nedarbo lygis (%)")

plot_nedarbas_region

pokytis <- rep(0, length(data3$NedarboLygis))

i=1
for(i in 1:length(data3$NedarboLygis)){
  pokytis[i] <- data3$NedarboLygis[i] - data3$NedarboLygis[i+1]
}

data3 %>% filter(Metai == 2011) %>% select(NedarboLygis) %>% summarise(vid = mean(NedarboLygis))

data3 %>% filter(Metai == 2016) %>% select(NedarboLygis) %>% summarise(vid = mean(NedarboLygis))
...
neuzimtu_vid <- mean(data6$n)

plot_neuzimtu_amzius <- ggplot(data = data6, aes(x = AGE, y = n)) +
  geom_bar(stat="identity") +
  theme(axis.text.x = element_text(angle = 35, hjust = 1, vjust = 0.5)) +
  labs(title = "Neužimtų žmonių pasiskirstymas pagal amžių",
       subtitle = "Raudona tiesė - vidutinis neuž. Neužimtas - bedarbis arba neaktyvus",
       x = "Amžius", y = "Dalis visų neužimtų (%)") +
  geom_hline(yintercept = neuzimtu_vid, color = "red")
...
plot_uzimtu_amzius <- ggplot(data = data6, aes(x = AGE, y = n)) +
  geom_bar(stat="identity") +
  theme(axis.text.x = element_text(angle = 35, hjust = 1, vjust = 0.5)) +
  labs(title = "Užimtų žmonių pasiskirstymas pagal amžių",
       x = "Amžius", y = "Dalis visų neužimtų (%)")
...
a<-anova(modLR.reduced, modLR, test = "Chisq")
a
```
